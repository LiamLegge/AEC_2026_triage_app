...
// Macro to get io_context for older/newer versions
...
namespace crow
{
    ...
    /// A wrapper for TCP sockets
    struct SocketAdaptor
    {
        using context = void;
        SocketAdaptor(asio::io_context& io_context, context* = nullptr)
            : socket_(io_context)
        {}
        ...
        void close() { error_code ec; socket_.close(ec); }
        void shutdown_readwrite() { error_code ec; socket_.shutdown(asio::socket_base::shutdown_both, ec); }
        void shutdown_write() { error_code ec; socket_.shutdown(asio::socket_base::shutdown_send, ec); }
        void shutdown_read() { error_code ec; socket_.shutdown(asio::socket_base::shutdown_receive, ec); }
    ...
    {
        using context = void;
        UnixSocketAdaptor(asio::io_context& io_context, context* = nullptr)
            : socket_(io_context)
        {}
        ...
        stream_protocol::endpoint remote_endpoint() { return socket_.local_endpoint(); }
    ...
    struct SSLAdaptor
    {
        ...
        std::string address() const { return ssl_socket_->lowest_layer().remote_endpoint().address().to_string(); }
        bool is_open() { return ssl_socket_ ? raw_socket().is_open() : false; }

        void close() { if (is_open()) { error_code ec; raw_socket().close(ec); } }
        void shutdown_readwrite() { if (is_open()) { error_code ec; raw_socket().shutdown(asio::socket_base::shutdown_both, ec); } }
        void shutdown_write() { if (is_open()) { error_code ec; raw_socket().shutdown(asio::socket_base::shutdown_send, ec); } }
        void shutdown_read() { if (is_open()) { error_code ec; raw_socket().shutdown(asio::socket_base::shutdown_receive, ec); } }
